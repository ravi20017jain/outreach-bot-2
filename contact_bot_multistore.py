...
# LocalTuneup ka message - personalized line {intro} ke through inject hoga
SUBJECT_TEMPLATE = "Managing Google listings for all your locations from one place"

# {intro} = AI-generated personalized opening line (ya khaali agar generate na ho)
MESSAGE_TEMPLATE = "Hi,\n\n{intro}Running multiple locations means multiple Google Business Profiles - and keeping all of them consistent (correct hours, fresh photos, posts, review replies) is nearly impossible to do manually as you grow.\n\nUsually some locations rank well while others stay invisible, reviews pile up unanswered, and head office has no clear view of which store is performing where.\n\nLocalTuneUp manages every location's Google profile from one place - consistent listings, posts, and review replies across all stores, plus one dashboard showing how each location ranks locally.\n\nFree 14-day trial, no card needed. Reply YES and we'll show you a quick before/after for your locations.\n\n- Team LocalTuneUp\nlocaltuneup.com"
...
def get_page_text(page):
    """Site ka visible text nikaalo - business kya karta hai samajhne ke liye."""
    try:
        txt = page.evaluate(
            """() => {
                const isVisible = (el) => {
...
def generate_personalized_line(page, website):
    """
    Site ka content padhke 1 short, specific personalized opening line banao.
    Ye line fixed message ke aage judegi. Fail hone par "" return karega (safe fallback).
    """
    site_text = get_page_text(page)
    if len(site_text.strip()) < 40:
        return ""  # Itna content nahi ki personalize kar sakein

    prompt = """You are writing the FIRST sentence of a cold outreach message to a multi-location business / chain / franchise.
...
Rules:
- Prefer to mention something that signals SCALE or MULTIPLE LOCATIONS: number of branches/outlets/stores, the cities/areas they operate in, their expansion, or that they're a growing chain/franchise. If you can see specific locations or a store count, name them.
- If no multi-location signal is visible, mention their main service or specialty instead.
- Sound human and warm, NOT salesy or generic. No "I hope this finds you well".
- Do NOT mention SEO, Google ranking, listings, or our offer - that comes later.
- Do NOT use the owner's name (we don't know it).
...
Example good output: Saw you've grown to outlets across Indore, Bhopal and Bengaluru -"""
    ...
    # Cleanup - quotes/markdown hatao, ek hi line rakho
    line = raw.replace("```", "").strip().strip('"').strip("'").strip()
    line = line.split("\n")[0].strip()
    # Safety: agar AI galti se SEO/offer mention kare ya bahut lamba ho to skip
    ...
    with sync_playwright() as p:
        ...
        for row_idx, row_data in to_process:
            ...
            try:
                ...
                # --- PERSONALIZE: homepage pe rehte hue site ka content padhke
                #     personalized opening line banao (contact page pe jaane se PEHLE,
                #     kyunki homepage pe business ka content hota hai) ---
                try:
                    intro_line = generate_personalized_line(pg, website)
                except Exception as e:
                    log.warning("  [Personalize] error: {}".format(str(e)[:50]))
                    intro_line = ""
                # Line ke baad space/double-newline daalo taaki message saaf flow kare
